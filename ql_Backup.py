...
def start():
    ...
    files_name = f'{QLBK_BACKUPS_PATH}/ql_{now_time}.tar.gz'
    ...
    if make_targz(files_name, retval):
        ...
        text = f'已备份至阿里网盘:\n{run_path}{QLBK_BACKUPS_PATH}/ql_{now_time}.tar.gz\n' \
               f'\n备份完成时间:\n{message_up_time}\n'
        try:
            send('【ql备份】', text)
        except:
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info('登录失败')
        try:
            send('【ql自动备份】', '阿里网盘登录失败,请手动重新运行本脚本登录')
        except:
            logger.info("通知发送失败")
        sys.exit(1)
    start()
